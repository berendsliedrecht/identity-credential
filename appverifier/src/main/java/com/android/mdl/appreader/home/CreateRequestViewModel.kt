...
class CreateRequestViewModel : ViewModel() {
    ...
    fun onRequestUpdate(fieldsRequest: DocumentElementsRequest) {
        ...
        when (updated.title) {
            ...
            state.value.utrechtInteropEventPid.title -> mutableState.update { it.copy(utrechtInteropEventPid = updated) }
            state.value.utrechtInteropEventMdl.title -> mutableState.update { it.copy(utrechtInteropEventMdl = updated) }
        }
    }
